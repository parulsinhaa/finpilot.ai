...
def llm_decide_action(state: dict) -> dict:
    """Use OpenAI client to decide the next financial action."""
    prompt = (
...
def _rule_based_action(state: dict) -> dict:
    ...
    return {"action": {"type": "save", "params": {"amount": max(round(surplus*0.4,2), 1000)}},
            "reasoning": f"Stable — saving from monthly surplus.", "source": "rule_based"}
...
def run_episode(task: str = "wealth_building", seed: int = 42) -> float:
    ...
    for step_idx in range(env.task_config["max_steps"]):
        ...
        print(json.dumps({
            ...
            "observation": {
                "net_worth":       round(new_state["net_worth"], 2),
                "savings":         round(new_state["savings"], 2),
                "debt":            round(new_state["debt"], 2),
                "investments":     round(new_state["investments"], 2),
                "emergency_fund":  round(new_state["emergency_fund"], 2),
    ...
    print(json.dumps({
        ...
        "final_observation": {
            "net_worth":    round(final_state["net_worth"], 2),
            "savings":      round(final_state["savings"], 2),
            "debt":         round(final_state["debt"], 2),
            "investments":  round(final_state["investments"], 2),
            "health_score": final_state["health_score"],
            "streak_days":  final_state.get("streak_days", 0),
        },
        "score": {
            "episode_reward":  round(episode_reward, 6),
            "step_avg_reward": round(total_reward / max(len(history), 1), 6),
            "normalised":      round(episode_reward, 6),
            "range":           [0.0, 1.0],
        },
        "success": success,
        "performance_tier": (
            ...
            "fair"      if episode_reward >= 0.40 else
            "poor"
        ),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", default="wealth_building",
                        choices=["budget_balance", "debt_payoff", "wealth_building"])
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()
    reward = run_episode(task=args.task, seed=args.seed)
    sys.exit(0 if reward >= 0.3 else 1)